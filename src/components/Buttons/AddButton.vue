          ...
          <v-form ref="form" v-model="valid" lazy-validation @submit.prevent="addClass">
            ...
            <div v-for="(field, index) in fields" :key="index" class="d-flex justify-space-between">
              ...
              <v-select
                ...
                :style="{ maxWidth: fields.length != 1 ? '45%' : '48%' }"
              ></v-select>         
              <v-icon v-if="fields.length != 1" size="24" color="red darken-2" @click="removeField(index)">mdi-close-box</v-icon>
            </div>
            <v-btn class="d-block" style="width: 100%" outlined color="grey lighten-1" @click="addField">Добавить поле</v-btn>
          </v-form>
        </v-card-text>
...
<script>
const validName = /^[a-zA-z][a-zA-Z\d_]+$/;
